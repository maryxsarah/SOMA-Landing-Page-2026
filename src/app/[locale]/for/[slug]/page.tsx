...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) =>
    spokesInBucket('for').map((s) => ({ locale, slug: s.slug })),
  );
}
...
export async function generateMetadata(ctx: {
  ...
  const spoke = findSpoke('for', slug);
  if (!spoke) return {};
...
export default async function ForSpoke(ctx: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await ctx.params;
